...
function DashboardContent({ children }: { children: React.ReactNode }) {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col md:flex-row w-full max-w-[1600px] h-full md:h-[calc(100vh-100px)] max-md:max-h-none md:max-h-[900px] items-stretch md:items-center gap-4 md:gap-6 mt-16 md:mt-0">
        <Sidebar user={session.user as any} isOpen={false} onClose={() => {}} />
        <main className="flex-1 min-w-0 bg-white/10 backdrop-blur-[40px] border-t border-white/20 md:border rounded-none md:rounded-[32px] p-4 sm:p-6 md:p-8 flex flex-col text-white shadow-[0_8px_32px_rgba(0,0,0,0.3)] overflow-hidden relative">
